...
type RegistrarAluno={
 map(arg0: (RegistrarAluno:any)=>React.JSX.Element):React.ReactNode
 rm: number,
 nome: string,
 email: string,
 senha: string,
...
export default function AlunoCadastro(){
    const {id} = useParams();
    const [regaluno, setRegistraAluno] = React.useState<RegistrarAluno|null>(null);
    ...
    axios.interceptors.request.use(config => {
      ...
      useEffect(()=>{
        axios.post('http://10.5.9.9:38000/app/aluno/cadastra')
    
        .then(response => {
          setRegistraAluno (response.data);
...
<form className=" w-1/3 h-full gap-2 flex flex-col ml-auto mr-auto mt-24" >
  ...
  <div className="relative z-0 w-full mb-6 group">
      <input type="text" name="rmaluno" id="rmaluno" className="block pl-8 rounded-3xl py-2.5 px-0 w-full text-lg text-gray-900 bg-transparent border-2 border-gray-300 appearance-nblack dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
  ...
  <div className="relative z-0 w-full mb-6 group">
      <input type="email" name="emailaluno" id="emailaluno" className="block pl-8 rounded-3xl py-2.5 px-0 w-full text-lg text-gray-900 bg-transparent border-2 border-gray-300 appearance-nblack dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
    ...
    <div className="relative z-0 w-full mb-6 group">
        <input type="tel" name="telefonealuno" id="telefonealuno" className="block rounded-3xl py-2.5 px-0 w-full text-lg text-gray-900 bg-transparent border-2 pl-8 border-gray-300 appearance-nblack dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
    ...
    <div className="relative z-0 w-full mb-6 group">
        <input name="qntdalrturma" id="qntdalrturma" className="block pl-8 rounded-3xl py-2.5 px-0 w-full text-lg text-gray-900 bg-transparent border-2 border-gray-300 appearance-nblack dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
        <label htmlFor="qntdalrturma" className="peer-focus:font-medium pl-8 absolute text-lg text-black dark:text-gray-400 duration-300 transhtmlForm -translate-y-9 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-95 peer-focus:-translate-y-9">Turma:</label>
    </div>
