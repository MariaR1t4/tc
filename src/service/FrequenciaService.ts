...
export default class FrequenciaService{
    ...
    public async saveFrequencia(token_aluno: string, numero:number){
       try{
        ...
        const buscar_aula = await AulaRepository.findOneBy({id_aula:token_aluno})
        console.log(buscar_aula)
    ...
    public async findFrequencia(id_frequencia:string) : Promise<Frequencia | null> {
